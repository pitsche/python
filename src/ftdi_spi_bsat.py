    ...
    def __init__(self, parent, controller):
        ...
        def updatePorts():
            try:
                ...
                p1wr = p1_upd.get()
                ft_write(dev, (0x80, 0x00, OPS))  # clear CS0
                ft_write_cmd_bytes(dev,0x11, [(p0wr<<7) + (slv<<2) + 0,0,tx_p0[0],tx_p0[1]]) #initial address write only
                ft_write_cmd_bytes(dev,0x31, [(p0wr<<7) + (slv<<2) + 1,0,tx_p0[2],tx_p0[3]]) #read first, write second...
                ...
                p1_upd.set(False)
                rx0_32 = rx[2]*(2**24) + rx[3]*(2**16) + rx[6]*(2**8) + rx[7] #generate integer range 0 to 2**32
        ...
        def readMem():
            try:
                ...
                for i in range(0,numWords.get()):
                    ft_write_cmd_bytes(dev, 0x31, [(slv<<2),(0xb0 + startAddr.get() + i + 1),0,0])       #address first read
                    rx = ft_read(dev, 4)
